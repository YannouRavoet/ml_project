...
from dynamics import replicator, boltzmann_faqlearning, MultiPopulationDynamics
...
def _biased_rock_paper_scissors_easy():  # ZERO-SUM
    return pyspiel.create_matrix_game("biased_rock_paper_scissors", "Biased Rock Paper Scissors",
                                      # Rock    Paper   Scissors
...
def _phaseplot(games, bstreamplot=False):
    ...
    for g, game in enumerate(games):
        ...
        dyn = MultiPopulationDynamics(payoff_tensor, replicator) if is_2x2 else SinglePopulationDynamics(payoff_tensor,
                                                                                                         replicator)

        ## PLOTTING
        ax = plt.subplot2grid((1, len(games)), (0, g), projection="2x2") if is_2x2 else plt.subplot2grid(
            (1, len(games)), (0, g), projection="3x3")
...
def _trajectoryplot(game, population_histories, k=1):
    ...
    if is_2x2:
        payoff_tensor = game_payoffs_array(game)
        dyn = MultiPopulationDynamics(payoff_tensor,
                                      replicator)  # eps = replicator / boltz = boltzmann_qlearning / faq = boltzmann_faqlearning
        # dyn = LenientMultiPopulationDynamics(payoff_tensor, boltzmann_faqlearning, k=k)         # for de lfaq plots
...
def _dynamics_kplot(k_values, games):
    ...
    for k_, k in enumerate(k_values):
        ...
        plt.text(0.5, 1, "k = " + str(k), fontsize=24, horizontalalignment='center', verticalalignment='top',
                 fontweight='bold')
        plt.axis('off')
    plt.show()
