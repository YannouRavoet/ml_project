...
import epsilongreedy_QLearner
import boltzmann_FAQLeaner
import boltzmann_LFAQLearner
import boltzmann_QLearner
...
flags.DEFINE_float("lr", 0.001,
                   "learning rate")  # options:   0.001       0.01        0.1         0.5    a lower lr means slower convergence = prettier plots
...
flags.DEFINE_float("expl_min", 0.01,
                   "minimum exploration value")  # options:   0           0.003       0.003       0.003
flags.DEFINE_float("beta", 0.001,
                   "(frequency adjusted) beta-value")  # options:   /           /           0.01        0.01
flags.DEFINE_integer("k", 5, "(lenient) k-value")  # options:   /           /           /           8
flags.DEFINE_integer("train_iter", int(3e3),
                     "number of training iterations")  # options:   5e2         5e2         1e4         5e5
flags.DEFINE_integer("pop_iter", 6,
                     "number of times to train a set of agents")  # options:   7           7           10          4
...
def _env_play_episode(env, agents, evaluating=False):
    time_step = env.reset()
    agent_outputs = [agent.step(time_step, is_evaluation=evaluating) for agent in
                     agents]  # agent step (no training since no last state information) => only action selection
        ...
        for _ in range(FLAGS.pop_iter):
            ...
            for idx in range(env.num_players):
                if FLAGS.learner == "eps":
                    agents.append(epsilongreedy_QLearner.EpsilonGreedy_QLearner(player_id=idx, num_actions=num_actions,
                                                                                step_size=FLAGS.lr, discount_factor=1,
                                                                                epsilon=FLAGS.expl,
                                                                                epsilon_annealing=FLAGS.expl_ann,
                                                                                epsilon_min=FLAGS.expl_min))
                elif FLAGS.learner == "boltz":
                    agents.append(
                        boltzmann_QLearner.Boltzman_QLearner(player_id=idx, num_actions=num_actions, step_size=FLAGS.lr,
                                                             discount_factor=1, temperature=FLAGS.expl,
                                                             temperature_annealing=FLAGS.expl_ann,
                                                             temperature_min=FLAGS.expl_min))
                elif FLAGS.learner == "faq":
                    agents.append(boltzmann_FAQLeaner.Boltzmann_FAQLearner(player_id=idx, num_actions=num_actions,
                                                                           step_size=FLAGS.lr, discount_factor=1,
                    ...
                    agents.append(boltzmann_LFAQLearner.Boltzmann_LFAQLearner(player_id=idx, num_actions=num_actions,
                                                                              step_size=FLAGS.lr, discount_factor=1,
                                                                              temperature=FLAGS.expl,
                                                                              temperature_annealing=FLAGS.expl_ann,
                                                                              temperature_min=FLAGS.expl_min,
                                                                              beta=FLAGS.beta, k=FLAGS.k))
